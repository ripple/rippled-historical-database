...
module.exports = hbase => {
  ...
  function createReports(data) {
    ...
    console.log('main net ledgers:', totalMainLedgers);
    console.log('alt net ledgers:', totalAltLedgers);
    console.log('total ledgers:', Object.keys(data.ledgers).length);
  ...
  function saveReports(validators) {
    ...
    return Promise.map(Object.keys(validators), pubkey => {
      ...
      return hbase.putRow('validator_reports', rowkey, v);
    }).then(() => {
      console.log(Object.keys(validators).length + ' reports saved.');
    });
  }
  ...
  return {
    generateReports: date => {
      ...
      console.log('Generating reports for ' + start.format('YYYY-MM-DD'), new Date());
      getValidations(start, end)
