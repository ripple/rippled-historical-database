...
const Validations = function(hbaseConfig, configFile) {
  ...
  function verifyDomains() {
    const config = nconf.file(configFile);

    return addDomains(config.get('validator-domains'))
    .then(() => {
      return removeDomains(config.get('unverified-validators'))
      .then(() => {
