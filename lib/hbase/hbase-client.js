...
HbaseClient.prototype.getLedger = function (options, callback) {
    ...
    self.getRow({
      ...
      if (ledger.transactions.length &&
        ...
        self.getTransactions({
          ...
          } else if (hashes.length !== resp.length && !options.invalid) {
  ...
  function compare(a, b) {
    if (a.meta.TransactionIndex < b.meta.TransactionIndex) {
      return -1;
    } else if (a.meta.TransactionIndex > b.meta.TransactionIndex) {
      return 1;
    } else {
      return 0;
    }
...
HbaseClient.prototype.getTransactions = function(options, callback) {
  ...
  self.getRows({
    ...
    resp.forEach(function(tx) {
      try {
        tx.hash = tx.rowkey;
        tx.date = moment.unix(tx.executed_time).utc().format();
        tx.ledger_index = parseInt(tx.ledger_index, 10);

        if (options.binary) {
          tx.tx = tx.raw;
        } else {
          tx.tx = new SerializedObject(tx.raw).to_json();
          tx.meta = new SerializedObject(tx.meta).to_json();
        }
