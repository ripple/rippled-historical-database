  ...
  if (options.interval && options.interval !== 'day') {
    if (options.interval === 'week') {
      ...
      while (start.moment.diff(options.end.moment) <= 0) {
        keys.push(base + '|' + start.hbaseFormatStartRow());
    ...
    startRow = base + '|' + options.start.hbaseFormatStartRow();
    endRow = base + '|' + options.end.hbaseFormatStopRow();
  ...
  function handleResponse(err, resp) {
    if (resp && resp.rows) {
      resp.rows.forEach(function(row, i) {
        ...
        resp.rows[i] = {
          date: utils.unformatTime(parts[2]).format('YYYY-MM-DDTHH:mm:ss'),
          amount: amount
        };
      });
    }
