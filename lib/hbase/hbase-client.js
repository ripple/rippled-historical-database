...
HbaseClient.prototype.getPayments = function(options, callback) {
  var table = 'payments';
  var startRow = options.start.hbaseFormat();
  var endRow = options.end.hbaseFormat();
  var filters = [];
  var filterString;
...
HbaseClient.prototype.getAccountTransactions = function(options, callback) {
  ...
  if (options.minSequence || options.maxSequence) {
    ...
    startRow = options.account + '|' + utils.formatTime(options.start || 0);
...
HbaseClient.prototype.getExchangeRate = function(options) {
  ...
  function getDailyRate() {
    return new Promise(function(resolve, reject) {
      var start = moment.utc(options.date).startOf('day');
      self.getExchanges({
        base: options.base,
  ...
  function getLatestRate() {
    return new Promise(function(resolve, reject) {
      var start = moment.utc(options.date).subtract(14, 'days');
      self.getExchanges({
        base: options.base,
        counter: options.counter,
        start: start,
        ...
        if (err) {
          ...
          resolve(resp.reduced ? Number(resp.reduced.vwap || 0) : 0);
        }
      });
    });
  }
...
HbaseClient.prototype.getTransactions = function(options, callback) {
  ...
  function getTransactionsByTime(opts, cb) {
    ...
    self.getScanWithMarker(self, {
      ...
      startRow: utils.formatTime(opts.start),
      stopRow: utils.formatTime(opts.end),
      marker: opts.marker,
