...
Client.prototype.saveTransaction = function (tx, callback) {
  var self   = this;
  var data   = [ ];
  ...
  .nodeify(function(err, resp) {  
    if (err) {
      log.error('error saving transaction:', ledger_index, tx.hash, err);
    } else {
      log.info('transaction saved:', ledger_index, tx.tx_index);
    }
