...
HbaseClient.prototype.getScan = function (options, callback) {
  ...
  self._getConnection(function(err, connection) {
    ...
    var potentialFilters =
      [{ k: 'base_currency', v: options.base },
       { k: 'base_issuer' , v: options.base_issuer },
       { k: 'counter_currency', v: options.counter },
       { k: 'counter_issuer' , v: options.counter_issuer }];

    var filterString= potentialFilters.map( function( obj ) {
        if(obj.v) {
          return ["SingleColumnValueFilter ('f', '", obj.k, "', =, 'binary:", obj.v, "')"].join('');
        }
     }).filter(function(n){ return n != undefined }).join(' AND ');

    if(filterString!="") {
      scanOpts.filterString = filterString;
    }
