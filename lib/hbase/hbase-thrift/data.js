...
HbaseClient.getFeeStats = function(options) {
  ...
  if (options.interval) {
    var start = options.descending ? options.end.format() : options.start.format();
    var end = options.descending ? options.start.format() : options.end.format();
      ...
      self.getRows({
        ...
        if (err) {
          ...
          if (resp.length > options.limit) {
            ...
            resp = resp.splice(0, options.limit);
          }
