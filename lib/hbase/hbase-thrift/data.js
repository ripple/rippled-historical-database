...
HbaseClient.getGateways = function(params) {
  var self = this;
  var options = {
    table: 'gateways',
    startRow: ' ',
  ...
  if (params && params.issuer) {
    options.limit = 1;
    options.filterString = this.buildSingleColumnValueFilters([
      {
        qualifier: 'address',
        value: params.issuer,
        family: 'f', comparator: '='
    ...
    .then(function(d) {
      return d.length ? self.getGateways({
        normalized_name: d[0].normalized_name
      }) : undefined;
    });

  } else if (params && params.normalized_name) {
    options.filterString = this.buildSingleColumnValueFilters([
      {
        qualifier: 'normalized_name',
        value: params.normalized_name,
        family: 'f', comparator: '='
      }
    ]);
    ...
    .then(function(resp) {
      ...
      var gateway = {
        ...
        accounts: [],
        hotwallets: [],
        start_date: resp[0].start_date,
        domain: resp[0].domain,
        normalized: resp[0].normalized_name
      }

      resp.forEach(function(d) {
        if (d.type === 'hot wallet') {
          if (gateway.hotwallets.indexOf(d.address) === -1) {
            gateway.hotwallets.push(d.address);
          }

        } else if (d.type === 'issuer') {
          if (!accounts[d.address]) {
            accounts[d.address] = {
              address: d.address,
              currencies: { }
            }
          ...
          accounts[d.address].currencies[d.currency] = {
            featured: d.featured
          };
        } else {
          console.log(d);
      ...
      gateway.accounts = Object.keys(accounts).map(function(d) {
        return accounts[d];
      });
  ...
  function getData(d) {
    return new Promise(function(resolve, reject) {
      self.getScan(d, function(err, resp) {
        if (err) {
          reject(err);
        } else {
