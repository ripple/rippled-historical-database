...
var Validator = function(config) {
  ...
  function checkNextLedger() {
    ...
    function (err, ledger) {
      ...
      try {
        txHash = ripple._DEPRECATED.Ledger.from_json(ledger).calc_tx_hash().to_hex();
      } catch(e) {
      ...
      if (txHash !== ledger.transaction_hash) {
        message = 'transactions do not hash to the expected value for ' +
  ...
  function importLedger (ledger_index, callback) {
    ...
    importer.getLedger({index : ledger_index}, function (err, ledger) {
      if (err) {
        ...
        } else if (err.indexOf('ledger not validated') !== -1) {
