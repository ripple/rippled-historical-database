...
var Payments = function (tx) {
  ...
  if (payment.currency !== 'XRP') {
    for (var i=0; i<payment.destination_balance_changes.length; i++) {
      if (payment.destination_balance_changes[i].currency === payment.currency) {
        payment.issuer = payment.destination_balance_changes[i].counterparty;
        break;
      }
    }
  }
