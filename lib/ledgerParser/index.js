...
Parser.parseHBase = function (ledger) {
  ...
  var ledgerTimeKey  = utils.reverseTimestamp(ledger.close_time) + '|' + pad(data.ledger.ledger_index, LI_PAD);
  ...
  data.exchanges.forEach(function(ex) {
    ...
    tables.exchanges[key] = {
      ...
      rate              : ex.rate,
      'f:account'       : ex.account,
      'f:counterparty'  : ex.counterparty,
  ...
  data.accountsCreated.forEach(function(a) {
    var key = utils.reverseTimestamp(a.time) + 
  ...
  data.memos.forEach(function(m) {
    var key = utils.reverseTimestamp(m.time) + 
