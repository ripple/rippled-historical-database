...
Parser.parseHBase = function (ledger) {
  ...
  var ledgerTimeKey  = utils.formatTime(ledger.close_time) + '|' + data.ledger.ledger_hash;
  ...
  tables.ledgers_by_index[ledgerTimeKey] = {
    ledger_hash : data.ledger.ledger_hash,
  ...
  data.payments.forEach(function(p) {
    ...
    tables.payments_by_account[p.sender   + '|' + key] = payment;
    tables.payments_by_account[p.receiver + '|' + key] = payment;
  });
